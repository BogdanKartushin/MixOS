...
static DWORD rgbText = RGB(255, 255, 255);
...
static LRESULT CALLBACK wnd_proc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE: {
        ...
        HICON hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\MixOS.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
        ...
        hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\Emulate.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
        ...
        hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\menumaker.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
        ...
        hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\quest.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
        ...
        hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\piano.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
        ...
        hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\terminal.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
        ...
        hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\youtube.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
        ...
        BASS_Start();
        START = BASS_StreamCreateFile(FALSE, L"MusicPlayer\\Your\\Mozart.mp3", 0, 0, 0);
        BASS_ChannelPlay(START, false);


        hTrack1 = CreateWindow(TRACKBAR_CLASS, L"SOUND", WS_CHILD | TBS_AUTOTICKS | TBSTYLE_TOOLTIPS /*| LVS_EX_TRANSPARENTBKGND*/ | WS_VISIBLE | TBS_ENABLESELRANGE, 10, 20, 100, 20, hWnd, (HMENU)LENMUSIC4, NULL, NULL);
        ...
        CreateWindow(L"BUTTON", L"Установить скин", WS_VISIBLE | WS_CHILD, 120, 80 - 30 - 30, 150, 20, hWnd, (HMENU)SETBKG, NULL, NULL);



        hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\instskin.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
        ...
        CreateWindow(L"BUTTON", L"Установить полупрозрачность", WS_VISIBLE | WS_CHILD, 120, 80 - 30 - 30 + 30, 320, 20, hWnd, (HMENU)polupon, NULL, NULL);

        hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\desktop.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);

        SendMessage(GetDlgItem(hWnd, polupon), BM_SETIMAGE, IMAGE_ICON, (LPARAM)hIconAll);


        CreateWindow(L"BUTTON", L"Выключить полупрозрачность", WS_CHILD, 120, 80 - 30 - 30 + 30, 320, 20, hWnd, (HMENU)polupoff, NULL, NULL);

        hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\computer.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
        ...
        CreateWindow(L"BUTTON", L"Удалить скин", WS_VISIBLE | WS_CHILD, 120 + 150 + 20, 80 - 30 - 30, 150, 20, hWnd, (HMENU)CLEARSKIN, NULL, NULL);

        hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\delete.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
    ...
    case WM_COMMAND: {
        ...
        if (LOWORD(wParam) == ID_BUTTON)
        {
           ...
           /* SendMessage(hWnd, WM_SETICON, NULL, (LPARAM)hIcon2);*/
            ...
            SetWindowText(hWnd, L"MixPlayer Pre Beta 2.3 из MixOS Lite");
            bmpwall1 = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\clear.png", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
            ...
            if (bmpwall != NULL);
            {
                hwall = CreateWindow(L"STATIC", NULL, WS_CHILD | WS_VISIBLE | SS_BITMAP | WS_BORDER, 0, 0, 0, 0, hWnd, NULL, NULL, NULL);
            ...
            BASS_Stop();
            BASS_Start();
            VISTA = BASS_StreamCreateFile(FALSE, L"MusicPlayer\\Your\\Vista.mp3", 0, 0, 0);
           ...
           /* Proga1.close();*/
            ...
            AppendMenu(hMenubar, MF_POPUP, (UINT_PTR)hAbout, L"Справка");
            AppendMenu(hMenubar, MF_POPUP, (UINT_PTR)hCreator, L"О разработчике");
            AppendMenu(hMenubar, MF_POPUP, (UINT_PTR)hHelp, L"Шашнахме");
            AppendMenu(hMenubar, MF_POPUP, (UINT_PTR)hUtils, L"Системные утилиты");
            AppendMenu(hMenubar, MF_POPUP, (UINT_PTR)hInstall, L"Установка полезного софта");
            AppendMenu(hMenubar, MF_POPUP|MF_DISABLED, (UINT_PTR)hRasvl, L"Развлечения");
            AppendMenu(hMenubar, MF_POPUP, (UINT_PTR)hOptions, L"Настройки");
            AppendMenu(hMenubar, MF_POPUP|MF_DISABLED, (UINT_PTR)hWhat, L"С чего начиналась разработка?");
            ...
            AppendMenu(hFile, MF_STRING, IU_STR, L"Выход");
            AppendMenu(hFile, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hFile, MF_STRING /*| MF_OWNERDRAW*/, MENUMENUMENUMENU, L"Вернуться в меню");

            AppendMenu(hFile, MF_STRING, SOURCE, L"Исходный код");
            AppendMenu(hFile, MF_STRING, FASTRESTART, L"Быстрая перезагрузка Windows");

            AppendMenu(hAbout, MF_STRING, STRANNIK, L"О программе");



            AppendMenu(hCreator, MF_STRING, WOZRAST, L"Возраст");
            AppendMenu(hCreator, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hCreator, MF_STRING, WEIGHT, L"Вес");
            AppendMenu(hCreator, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hCreator, MF_STRING, ROSTE, L"Рост");
            AppendMenu(hCreator, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hCreator, MF_STRING|MF_DISABLED, DUSHA, L"Душа");
            AppendMenu(hCreator, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hCreator, MF_STRING|MF_DISABLED, POL, L"Пол");
            AppendMenu(hCreator, MF_SEPARATOR, NULL, NULL);

            AppendMenu(hCreator, MF_STRING, ABOUTDEV, L"О процессе разработки");

            AppendMenu(hHelp, MF_STRING, HELP, L"Помощь");
            AppendMenu(hHelp, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hHelp, MF_STRING| MF_DISABLED, QUEST, L"Поиграть в шашнахме квест");
            AppendMenu(hHelp, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hHelp, MF_STRING| MF_DISABLED, PIANINO, L"Поиграть на пианино");
            AppendMenu(hHelp, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hHelp, MF_STRING, TERMINAL, L"Открыть терминал");
            AppendMenu(hHelp, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hHelp, MF_STRING| MF_DISABLED, CONVERTER, L"Открыть конвертер валют");
            AppendMenu(hHelp, MF_SEPARATOR, NULL, NULL);

            AppendMenu(hHelp, MF_STRING, MENUMAKER, L"Открыть контекстменюмейкер для Windows");
            AppendMenu(hHelp, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hHelp, MF_STRING, YOUTUBE, L"Открыть загрузчик видосов с ютуба");
            AppendMenu(hHelp, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hHelp, MF_STRING| MF_DISABLED, ERTOR, L"Открыть генератор случайных чисел");
            AppendMenu(hHelp, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hHelp, MF_STRING, VIDEO, L"Открыть видеоплеер");
            AppendMenu(hHelp, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hHelp, MF_STRING| MF_DISABLED, PHOTOVIEWER, L"Открыть просмотрщик фотографий");
            AppendMenu(hHelp, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hHelp, MF_STRING| MF_DISABLED, TRANSLATE, L"Открыть руингтранслит");
            //AppendMenu(hHelp, MF_SEPARATOR, NULL, NULL);
            //AppendMenu(hHelp, MF_STRING, SELFUP, "Открыть помощник по повышению самооценки (для девушек).");

            AppendMenu(hUtils, MF_STRING, NOTEPAD, L"Открыть блокнот");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, RSTRUI, L"Открыть восстановление системы");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, DEFRAG, L"Открыть дефрагментацию диска");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, TASKMGR, L"Открыть диспетчер задач");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, DEVICES, L"Открыть диспетчер устройств");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, CALC, L"Открыть калькулятор");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, RESMON, L"Открыть монитор ресурсов");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, CLEANMGR, L"Открыть очистку диска");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, PANEL, L"Открыть панель управления");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, PERSONALIZE, L"Открыть персонализацию");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);

            AppendMenu(hUtils, MF_STRING, GPEDIT, L"Открыть редактор групповых политик");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, REGEDIT, L"Открыть редактор реестра");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, SYSTEMA, L"Открыть свойства системы");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, COMPMGMT, L"Открыть управление компьютером");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, MSCONFIG, L"Открыть msconfig");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, MSINFO32, L"Открыть msinfo32");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, MSPAINT, L"Открыть paint");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, VERIFIER, L"Открыть verifier");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, MRT, L"Открыть MRT");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, EVENTVWR, L"Открыть просмотр событий");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, SERVICES, L"Открыть службы");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, TASKSCHD, L"Открыть планировщик заданий");
            AppendMenu(hUtils, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hUtils, MF_STRING, MDSCHED, L"Открыть средство проверки памяти Windows");
            ...
            AppendMenu(hInstall, MF_STRING| MF_DISABLED, VISUAL, L"Установить Visual C++ редисты");
            AppendMenu(hInstall, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hInstall, MF_STRING| MF_DISABLED, VISUALSTUDIO, L"Установить Visual Studio 2022");
            AppendMenu(hInstall, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hInstall, MF_STRING| MF_DISABLED, OBS, L"Установить OBS");
            AppendMenu(hInstall, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hInstall, MF_STRING, VEGAS, L"Открыть CPU-Z");
            AppendMenu(hInstall, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hInstall, MF_STRING, HW, L"Открыть HW monitor");
            AppendMenu(hInstall, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hInstall, MF_STRING| MF_DISABLED, BOMB, L"Открыть Revo Uninstaller");



            AppendMenu(hRasvl, MF_STRING| MF_DISABLED, EMULATE, L"Эмуляторы");
            AppendMenu(hRasvl, MF_STRING| MF_DISABLED, COMP4, L"Компьютерная эволюция 4");


            AppendMenu(hOptions, MF_STRING, MYCOLOR, L"Сменить цвет");
            AppendMenu(hOptions, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hOptions, MF_STRING | MF_DISABLED, MYFONT, L"Сменить шрифт (не реализовано)");
            AppendMenu(hOptions, MF_STRING, WHITETEXT, L"Белый цвет текста");
            AppendMenu(hOptions, MF_STRING, BLACKTEXT, L"Черный цвет текста");
            AppendMenu(hOptions, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hOptions, MF_STRING, MYBRUSH, L"Сделать рандомную фигню");
            AppendMenu(hOptions, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hOptions, MF_STRING, SETBKG, L"Сменить фон на картинку");
            AppendMenu(hOptions, MF_STRING, SKIN1, L"Картинка 1");
            AppendMenu(hOptions, MF_STRING, SKIN2, L"Картинка 2");
            AppendMenu(hOptions, MF_STRING, SKIN3, L"Картинка 3");
            AppendMenu(hOptions, MF_STRING, SKIN4, L"Картинка 4");
            AppendMenu(hOptions, MF_STRING, SKIN5, L"Картинка 5");
            AppendMenu(hOptions, MF_STRING, SKIN6, L"Картинка 6");
            AppendMenu(hOptions, MF_STRING, SKIN7, L"Картинка 7");
            AppendMenu(hOptions, MF_STRING, SKIN8, L"Картинка 8");
            AppendMenu(hOptions, MF_STRING, SKIN9, L"Картинка 9");
            AppendMenu(hOptions, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hOptions, MF_STRING, polupon, L"Включить полупрозрачность");
            AppendMenu(hOptions, MF_STRING, polupoff, L"Выключить полупрозрачность");
            AppendMenu(hOptions, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hOptions, /*WS_DISABLED|*/ MF_STRING | MF_DISABLED, LOADDLL, L"Загрузить плагины (не реализовано)");



            AppendMenu(hWhat, MF_STRING, OLDVER13, L"MusicPlayer Beta 1.3");
            AppendMenu(hWhat, MF_SEPARATOR, NULL, NULL);
            AppendMenu(hWhat, MF_STRING, OLDVER15, L"MusicPlayer Beta 1.5");
            ...
            hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\music.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
            ...
            hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\play.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
            ...
            hIconAll = (HICON)LoadImage(hInst, L"MusicPlayer\\BMP\\ico\\pause.ico", IMAGE_ICON, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
            ...
            hTrack = CreateWindow(TRACKBAR_CLASS, L"SOUND", WS_CHILD | TBS_AUTOTICKS | TBSTYLE_TOOLTIPS | LVS_EX_TRANSPARENTBKGND | TBS_ENABLESELRANGE, 450 - 150 - 100, 590 - 220, 320, 30, hWnd, (HMENU)LENMUSIC, NULL, NULL);
            SendMessage(hTrack, TBM_SETPOS, 0, 1);
            SendMessage(hTrack, TBM_SETRANGEMAX, TRUE, 100);


            hTrack2 = CreateWindow(TRACKBAR_CLASS, L"SOUND", WS_CHILD | TBS_AUTOTICKS | TBSTYLE_TOOLTIPS | LVS_EX_TRANSPARENTBKGND | TBS_ENABLESELRANGE, 450 + 150 + 100, 590 - 220, 320, 30, hWnd, (HMENU)LENMUSIC3, NULL, NULL);

            FOR6COMPOSIT = CreateWindow(TRACKBAR_CLASS, L"SOUND", WS_CHILD | TBS_AUTOTICKS | TBSTYLE_TOOLTIPS | LVS_EX_TRANSPARENTBKGND | TBS_ENABLESELRANGE, 450-150-100, 590 - 220, 320, 30, hWnd, (HMENU)LENMUSIC2, NULL, NULL);
            SendMessage(FOR6COMPOSIT, TBM_SETPOS, 0, 1);
            SendMessage(FOR6COMPOSIT, TBM_SETRANGEMAX, TRUE, 100);

            FOR6COMPOSIT1 = CreateWindow(TRACKBAR_CLASS, L"SOUND", WS_CHILD | TBS_AUTOTICKS | TBSTYLE_TOOLTIPS | LVS_EX_TRANSPARENTBKGND | TBS_ENABLESELRANGE | WS_VISIBLE, 450+150+100, 590-220, 320, 30, hWnd, (HMENU)LENMUSIC5, NULL, NULL);
            ...
            playlist = CreateWindow(/*WS_EX_NOACTIVATE,*/ L"LISTBOX", NULL, WS_CHILD | LBS_HASSTRINGS | WS_HSCROLL | WS_VSCROLL, 150, 150, 920, 180, hWnd, NULL, NULL, NULL);
            SendMessage(playlist, LB_INITSTORAGE, 0x7FFF, MAX_PATH); // выделяем память для списка
            UpdateWindow(playlist);
            CreateWindow(L"BUTTON", L"Выбрать/Проиграть" /*"Вставить файл"*/, WS_CHILD, (150 * 1.75) - 10, 330, 150, 30, hWnd, (HMENU)PLSEL, NULL, NULL);
            CreateWindow(L"BUTTON", L"Добавить файл", WS_CHILD, (310 * 1.75) - 10, 330, 150, 30, hWnd, (HMENU)PLADD, NULL, NULL);
            CreateWindow(L"BUTTON", L"Удалить файл", WS_CHILD, (470 * 1.75) - 10, 330, 150, 30, hWnd, (HMENU)PLDEL, NULL, NULL);
        ...
        {
            _wsystem(L"start C:\\WIN\\GAMES\\COMP4\\1.bat");


        }
        if (LOWORD(wParam) == TRANSLATE)
        {
            _wsystem(L"start MusicPlayer\\SUBS\\NBR\\ru4en.hta");
        ...
        if (LOWORD(wParam) == SETBKG)
        {
            ...
            //ofn.lpstrInitialDir = ".";
            ofnn.lpstrInitialDir = L"MusicPlayer\\BMP\\MIXSKIN";
...

        {
            ...
            bmpwall = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\image.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
            bmpwall1 = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\clear.png", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
...

        {
            ...
            bmpwall = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\image2.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
            bmpwall1 = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\clear.png", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
...

        {
            ...
            bmpwall = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\image3.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
            bmpwall1 = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\clear.png", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
...

        {
            ...
            bmpwall = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\image4.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
            bmpwall1 = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\clear.png", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
...

        {
            ...
            bmpwall = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\image5.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
            bmpwall1 = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\clear.png", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
...

        {
            ...
            bmpwall = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\image6.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
            bmpwall1 = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\clear.png", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
...

        {
            ...
            bmpwall = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\image7.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
            bmpwall1 = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\clear.png", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
...

        {
            ...
            bmpwall = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\image8.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
            bmpwall1 = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\clear.png", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
...

        {
            ...
            bmpwall = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\image9.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
            bmpwall1 = LoadImage(NULL, L"MusicPlayer\\BMP\\MIXSKIN\\clear.png", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT | LR_DEFAULTSIZE);
        ...
        if (LOWORD(wParam) == PLSEL)
        {
            ...
            SetWindowText(sbar, L"Проигрывается ваша музыка)");
        ...
        if (LOWORD(wParam) == PLADD)
        {
            ...
            ofn.nMaxFileTitle = 0;
            //ofn.lpstrInitialDir = ".";
            ofn.lpstrInitialDir = L"MusicPlayer\\Your";
   ...
   /*
        ...
        {
            _wsystem(L"start MusicPlayer\\MENUMAKER\\MENUMAKER.exe");

        }
        if (LOWORD(wParam) == ERTOR)
        {
            _wsystem(L"start MusicPlayer\\SUBS\\ERTOR\\gen.exe");

        }
        if (LOWORD(wParam) == PHOTOVIEWER)
        {
            _wsystem(L"start MusicPlayer\\PhotoViewer\\CreeperPhotoViewer.exe");
        }
        if (LOWORD(wParam) == SELFUP)
        {
            _wsystem(L"start MusicPlayer\\love\\helpup.exe");
        }
        if (LOWORD(wParam) == YOUTUBE)
        {
            _wsystem(L"start MusicPlayer\\Youtube\\Youtube.exe");
        }
        if (LOWORD(wParam) == HW)
        {
            _wsystem(L"start MusicPlayer\\Install\\HW\\HW.exe");
        }

        if (LOWORD(wParam) == VEGAS)
        {
            _wsystem(L"start MusicPlayer\\Install\\CPUZ\\cpuz64.exe");
        }

        if (LOWORD(wParam) == OBS)
        {
            _wsystem(L"start MusicPlayer\\Install\\OBS\\OBS.exe");
        }

        if (LOWORD(wParam) == VISUALSTUDIO)
        {
            _wsystem(L"start MusicPlayer\\Install\\VisualStudio\\Setup.exe");
        }
        if (LOWORD(wParam) == BOMB)
        {
            _wsystem(L"start MusicPlayer\\Progs\\REVO\\RevoUnin.exe");
        }
        if (LOWORD(wParam) == SPY)
        {
            _wsystem(L"start MusicPlayer\\Progs\\SHUTUP\\KILLSPY.exe");
        }
        if (LOWORD(wParam) == PANEL)
        {
            _wsystem(L"start control");
        }
        if (LOWORD(wParam) == VISUAL)
        {
            MessageBox(hWnd, L"После закрытия этого окна будут переустановлены все VISUAL C++ редисты, если вы не уверены, то отменить это можно чуть позже в консоли.", L"Внимание", MB_ICONINFORMATION);
            _wsystem(L"start MusicPlayer\\Install\\Visual\\Visual.bat");
        }
        if (LOWORD(wParam) == OLDVER13)
        {
            MessageBox(hWnd, L"Сейчас будет запущен плеер версии Beta 1.3. По сути это прадедушка MixOS. С него всё и началось.", L"MusicPlayer Beta 1.3", MB_ICONINFORMATION);
            _wsystem(L"start MusicPlayer\\OLD\\1.3.bat");
        }
        if (LOWORD(wParam) == OLDVER15)
        {
            MessageBox(hWnd, L"Сейчас будет запущен плеер версии Beta 1.5. Официально это первая графическая версия моего плеера.\nНеофициально это была Beta 1.4. ", L"MusicPlayer Beta 1.5", MB_ICONINFORMATION);
            _wsystem(L"start MusicPlayer\\OLD\\1.5.bat");
        }
        if (LOWORD(wParam) == TERMINAL)
        {
            _wsystem(L"start MusicPlayer\\TERMINAL\\TERMINAL.EXE");

        }

        if (LOWORD(wParam) == PIANINO)
        {
            _wsystem(L"start MusicPlayer\\Piano\\piano.exe");

        }
        if (LOWORD(wParam) == CONVERTER)
        {
            _wsystem(L"start MusicPlayer\\CONVERT\\converter.exe");

        }

        if (LOWORD(wParam) == EMULATE || LOWORD(wParam) == Emulators)
        {
            _wsystem(L"start MusicPlayer\\Emulators\\Emulate.exe");
        ...
        if (LOWORD(wParam) == DEVUSHKA)
        {
            ...
            SetWindowText(sbar, L"Проигрывается Headless Nick - Morning");
            free_samples_all();
            MORNING = BASS_StreamCreateFile(FALSE, L"MusicPlayer\\HeadlessNick\\Morning.mp3", 0, 0, 0);
        ...
        if (LOWORD(wParam) == LETO)
        {
            ...
            SetWindowText(sbar, L"Проигрывается Headless Nick - UFO ");
            free_samples_all();
            UFO = BASS_StreamCreateFile(FALSE, L"MusicPlayer\\HeadlessNick\\UFO.mp3", 0, 0, 0);
        ...
        if (LOWORD(wParam) == SHAG)
        {
            ...
            SetWindowText(sbar, L"Проигрывается Headless Nick - Sample Adventures.mp3");
            free_samples_all();
            SAMPLEADV = BASS_StreamCreateFile(FALSE, L"MusicPlayer\\HeadlessNick\\Sample Adventures.mp3", 0, 0, 0);
        ...
        if (LOWORD(wParam) == SLOVO)
        {
            ...
            SetWindowText(sbar, L"Проигрывается Deteez - Above the Sky.mp3");
            free_samples_all();
            EYES = BASS_StreamCreateFile(FALSE, L"MusicPlayer\\Violet S\\Above the Sky.mp3", 0, 0, 0);
        ...
        if (LOWORD(wParam) == CIGARET)
        {
            ...
            SetWindowText(sbar, L"Проигрывается Violet S - Lonely.mp3");
            free_samples_all();
            LONELY = BASS_StreamCreateFile(FALSE, L"MusicPlayer\\Violet S\\Lonely.mp3", 0, 0, 0);
        ...
        if (LOWORD(wParam) == PEREMEN)
        {
            ...
            SetWindowText(sbar, L"Проигрывается Captain Strannik - Hard Thing.mp3");
            free_samples_all();
            GIRL = BASS_StreamCreateFile(FALSE, L"MusicPlayer\\Your\\HTM.mp3", 0, 0, 0);
        ...
        if (LOWORD(wParam) == STOPS)
        {
            ...
            SetWindowText(sbar, L"Музыка выключена");
        ...
        {
            MessageBox(hWnd, L"Исходники лежат на гитхабе", L"Исходный код", MB_OK);
            _wsystem(L"start https://github.com/CaptainStrannik/MixOS");

        }
        if (LOWORD(wParam) == PAUSE1)
        {
            //CreateWindow(TEXT("STATIC"), TEXT("Пауза                                                        \n                                      "), WS_VISIBLE | WS_CHILD, 10, 600, 300, 50, hWnd, (HMENU)text1, NULL, NULL);
            SetWindowText(sbar, L"Пауза");
            ShowWindow(GetDlgItem(hWnd, PAUSE1), SW_HIDE);
        ...
        {
            _wsystem(L"start MusicPlayer\\VideoPlayer\\VideoPlayer.exe");
        }
        if (LOWORD(wParam) == CALC)
        {
            _wsystem(L"start calc.exe");
        }
        if (LOWORD(wParam) == TASKMGR)
        {
            _wsystem(L"start taskmgr.exe");
        }
        if (LOWORD(wParam) == CLEANMGR)
        {
            _wsystem(L"start cleanmgr.exe");
        }
        if (LOWORD(wParam) == REGEDIT)
        {
            _wsystem(L"start regedit.exe");
        }
        ...
        {
            _wsystem(L"start dfrgui.exe");
        }
        if (LOWORD(wParam) == GPEDIT)
        {
            _wsystem(L"start gpedit.msc");
        }
        if (LOWORD(wParam) == PERSONALIZE)
        {
            _wsystem(L"explorer shell:::{ED834ED6-4B5A-4bfe-8F11-A626DCB6A921}");
        }
        if (LOWORD(wParam) == RSTRUI)
        {
            _wsystem(L"start rstrui.exe");
        }
        if (LOWORD(wParam) == SYSTEMA)
        {
            _wsystem(L"explorer shell:::{bb06c0e4-d293-4f75-8a90-cb05b6477eee}");
        }
        if (LOWORD(wParam) == DEVICES)
        {
            _wsystem(L"start devmgmt.msc");
        }
        if (LOWORD(wParam) == MSCONFIG)
        {
            _wsystem(L"start msconfig.exe");
        }
        if (LOWORD(wParam) == RESMON)
        {
            _wsystem(L"start resmon.exe");
        }
        if (LOWORD(wParam) == NOTEPAD)
        {
            _wsystem(L"start notepad.exe");
        }
        if (LOWORD(wParam) == MSINFO32)
        {
            _wsystem(L"start msinfo32.exe");
        }
        if (LOWORD(wParam) == COMPMGMT)
        {
            _wsystem(L"start compmgmt.msc");
        }
        if (LOWORD(wParam) == VERIFIER)
        {
            _wsystem(L"start verifier.exe");
        }
        if (LOWORD(wParam) == MSPAINT)
        {
            _wsystem(L"start mspaint.exe");
        }
        if (LOWORD(wParam) == MRT)
        {
            _wsystem(L"start MRT.exe");
            //WinExec("MRT.exe", SW_SHOW);
        }
        if (LOWORD(wParam) == EVENTVWR)
        {
            _wsystem(L"start eventvwr.msc");
        }
        if (LOWORD(wParam) == SERVICES)
        {
            _wsystem(L"start services.msc");
        }
        if (LOWORD(wParam) == TASKSCHD)
        {
            _wsystem(L"start taskschd.msc");
        }
        if (LOWORD(wParam) == MDSCHED)
        {
            _wsystem(L"start mdsched.exe");
        }

        if (LOWORD(wParam) == MENUMENUMENUMENU)
        {

            _wsystem(L"start MixOS.exe");
            PostQuitMessage(0);

        }
        if (LOWORD(wParam) == FASTRESTART)
        {
            if (MessageBox(hWnd, L"Перед тем как перезагрузиться, обязательно закройте все программы!", L"Внимание!", MB_YESNO | MB_ICONWARNING) == IDYES)
                ExitWindowsEx(EWX_RESTARTAPPS, NULL);
        }

        if (LOWORD(wParam) == MYBRUSH)
        {
        ...
        if (LOWORD(wParam) == BLACKTEXT)
        {
            rgbText = RGB(0, 0, 0);
            InvalidateRect(hWnd, NULL, NULL);
        }
        if (LOWORD(wParam) == WHITETEXT)
        {
   ...
   // case WM_NCHITTEST:
    ...
    case WM_CTLCOLORSTATIC:
    {
        ...
        if (GetDlgCtrlID((HWND)lParam) == text1)
        {
            ...
            SetTextColor(hdcStatic, rgbText); // text color
...
int WINAPI wmain(/*int argc, wchar_t* argv[]*/)
{
    ...
    wc.lpszClassName = L"MixOS";
    wc.hCursor = LoadCursor(NULL, IDC_ARROW);
    progicon = (HICON)LoadImage(wc.hInstance, L"MusicPlayer\\BMP\\ico\\MixOS.ico", IMAGE_ICON, 48, 48, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
    wc.hIcon = progicon;
